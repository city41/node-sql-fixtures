...
FixtureGenerator.prototype.destroy = function destroy(callback) {
  var knex = this.knex;
  var tablesToCleanup = this._tablesToCleanup;

  if (!knex) {
    return bluebird.resolve().nodeify(callback);
  }

  if (!tablesToCleanup || !tablesToCleanup.length) {
    return this.disconnect(callback);
  }

  function deleteAll() {
    return knex(table).delete().onDelete('CASCADE');
  }

  return bluebird.each(tablesToCleanup, deleteAll)
    .bind(this)
