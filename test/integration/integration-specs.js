  ...
  describe("FixtureGenerator", function () {
      ...
      bluebird
        ...
        .then(function () {
          return knex.schema.createTable("simple_table", function (table) {
            ...
            table
              .string("auto_populated_column")
              .notNullable()
              .defaultTo("autopopulated");
          });
        })
        .then(function () {
        ...
        .then(function () {
          return knex.schema.createTable("has_foreign_key_to_itself", function (
            table
          ) {
            table.increments("id").primary();
            table.string("string_column");
        ...
        .then(function () {
          return knex.schema.createTable("has_two_foreign_keys", function (
            table
          ) {
            table.increments("id").primary();
    ...
    describe("selecting pre-existing data with unknown ID:s", function () {
          ...
          has_foreign_key: [
            {
              ...
              simple_table_id: {
                from: "simple_table",
                where: { string_column: wantedData },
              },
            },
            {
      ...
      it("should replace a single query object with its result, corresponding to a FK", function (done) {
        var knex = this.knex;
        this.fixtureGenerator
          .create(this.dataConfig)
          .then(function checkSimpleTableIdMatches(fixtures) {
            expect(
              fixtures.has_foreign_key[0].simple_table_id,
              "resolved FK"
            ).to.be.gt(0);
            ...
            knex("simple_table")
              ...
              .then(function (rowIdentifiedByFk) {
                expect(rowIdentifiedByFk, "rowIdentifiedByFk").to.have.property(
                  "string_column"
                );
      ...
      it("should fail early if FK lookup finds multiple rows", function (done) {
        this.dataConfig.has_foreign_key[0].simple_table_id.where = {
          string_column: "non-unique row",
        };
      ...
      it("should allow getting a column other than id from existing data", function (done) {
        ...
        this.fixtureGenerator
          ...
          .then(function (fixtures) {
            expect(fixtures.has_foreign_key[0].string_column).to.equal(
              "unique row"
            );
            done();
          })
    ...
    describe("generating fixtures", function () {
      it("should create a fixture with dependencies resolved", function (done) {
        ...
        this.fixtureGenerator.create(dataConfig).then(function (results) {
          ...
          expect(results.has_foreign_key[0].simple_table_id).to.eql(
            results.simple_table[0].id
          );
          ...
          knex("simple_table")
            .where("id", results.simple_table[0].id)
            .then(function (result) {
      ...
      it("should properly deal with same types at different priorities", function (done) {
        ...
        this.fixtureGenerator.create(dataConfig).then(function (results) {
          ...
          knex("simple_table")
            .whereIn("id", [
              results.simple_table[0].id,
              results.simple_table[1].id,
            ])
            .then(function (result) {
              expect(result[0].string_column).to.eql("value1");
              expect(result[1].string_column).to.eql("value2");

              knex("has_foreign_key")
                .where("id", results.has_foreign_key[0].id)
                .then(function (result) {
      ...
      it("should properly resolve same types at different priorities", function (done) {
        ...
        var knex = this.knex;
        this.fixtureGenerator.create(dataConfig).then(function (results) {
          expect(results.has_foreign_key_to_itself[1].parent_id).to.be.a(
            "number"
          );
          ...
          expect(
            results.has_two_foreign_keys[1].has_foreign_key_to_itself_id
          ).to.be.a("number");
          expect(
            results.has_two_foreign_keys[1].has_foreign_key_to_itself_id
          ).to.eql(results.has_foreign_key_to_itself[1].id);

          expect(results.has_two_foreign_keys[1].simple_table_id).to.be.a(
            "number"
          );
          expect(results.has_two_foreign_keys[1].simple_table_id).to.eql(
            results.simple_table[0].id
          );

          knex("has_two_foreign_keys")
            .where("id", results.has_two_foreign_keys[1].id)
            .then(function (result) {
              expect(result[0].simple_table_id).to.eql(
                results.simple_table[0].id
              );
              done();
            });
      ...
      it("should properly resolve dependencies that might move around", function (done) {
        ...
        this.fixtureGenerator.create(dataConfig).then(function (results) {
          expect(results.has_two_foreign_keys[0].string_column).to.eql(
            "value5"
          );
          expect(results.has_two_foreign_keys[1].string_column).to.eql(
            "value6"
          );
          expect(results.has_two_foreign_keys[1].simple_table_id).to.eql(
            results.simple_table[1].id
      ...
      it("should insert records in the same order as defined in the spec", function (done) {
        ...
        this.fixtureGenerator.create(dataConfig).then(function (results) {
          for (var i = 1; i < results.simple_table.length; ++i) {
            expect(results.simple_table[i].id).to.be.greaterThan(
              results.simple_table[i - 1].id
            );
          }
          done();
        });
      ...
      it("should properly treat an empty spec as a noop", function (done) {
        var dataConfig = {
          simple_table: [],
          has_foreign_key_to_itself: [],
        };
        ...
        this.fixtureGenerator.create(dataConfig).then(function (results) {
          ...
          expect(results.has_foreign_key_to_itself.length).to.equal(0);

          self.fixtureGenerator.create({}).then(function (results) {
            ...
            .then(function (firstResult) {
              this.fixtureGenerator
                ...
                .then(function (secondResult) {
                  expect(
                    secondResult.has_foreign_key[0].simple_table_id
                  ).to.not.equal(firstResult.simple_table[0].id);
                  expect(
                    secondResult.has_foreign_key[0].simple_table_id
                  ).to.not.equal(firstResult.simple_table[1].id);

                  expect(
                    secondResult.has_foreign_key[1].simple_table_id
                  ).to.not.equal(firstResult.simple_table[0].id);
                  expect(
                    secondResult.has_foreign_key[1].simple_table_id
                  ).to.not.equal(firstResult.simple_table[1].id);
                  ...
                  expect(
                    secondResult.has_foreign_key[1].simple_table_id
                  ).to.equal(secondResult.simple_table[1].id);

                  expect(firstResult.simple_table[0].id).to.not.equal(
                    secondResult.simple_table[0].id
                  );
                  expect(firstResult.simple_table[0].id).to.not.equal(
                    secondResult.simple_table[1].id
                  );
                  expect(firstResult.simple_table[1].id).to.not.equal(
                    secondResult.simple_table[0].id
                  );
                  expect(firstResult.simple_table[1].id).to.not.equal(
                    secondResult.simple_table[1].id
      ...
      describe("tables without an id column", function () {
        it("should create rows for tables without an id column", function (done) {
          ...
          this.fixtureGenerator.create(dataConfig).then(function (results) {
            ...
            expect(results.has_no_id_column[1]).to.not.have.property(
              "auto_populated_column"
            );

            knex("has_no_id_column")
              .where("foreign_a_id", 2)
      ...
      describe("invoking raw sql", function () {
        it("should invoke raw sql", function (done) {
          ...
          this.fixtureGenerator.create(dataConfig).then(function (results) {
            ...
            knex("has_foreign_key")
              .where("string_column", "rawsql0")
              .then(function (knexResult) {
                expect(knexResult[0].simple_table_id).to.eql(
                  results.simple_table[0].id
                );
                knex("has_foreign_key")
                  .where("string_column", "rawsql1")
                  .then(function (knexResult) {
                    expect(knexResult[0].simple_table_id).to.eql(
                      results.simple_table[1].id
                    );
      ...
      describe("auto populated columns", function () {
        it("should return the auto populated columns in the result if they are in the config", function (done) {
          ...
          this.fixtureGenerator.create(dataConfig).then(function (results) {
            expect(results.simple_table[0].auto_populated_column).to.eql(
              "autopopulated"
            );
            expect(results.simple_table[1]).to.not.have.property(
              "auto_populated_column"
            );
            done();
          });
    ...
    describe("errors", function (done) {
      it("should reject the promise if knex fails", function (done) {
        ...
        this.fixtureGenerator.create(dataConfig).then(
          function (result) {
